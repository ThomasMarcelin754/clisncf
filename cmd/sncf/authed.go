...
func authedClient() (*api.ConnectClient, error) {
	...
	if sess == nil {
		return nil, fmt.Errorf("not logged in — run `sncf auth login` first")
	}
	...
	if sess.Expired() {
		if sess.RefreshToken == "" {
			return nil, fmt.Errorf("session expired, no refresh token — run `sncf auth login`")
		}
		at, it, rt, expireIn, err := client.Refresh(sess.RefreshToken)
		if err != nil {
			return nil, fmt.Errorf("refresh failed: %w — run `sncf auth login`", err)
		}
		sess.AccessToken = at
		sess.IDToken = it
